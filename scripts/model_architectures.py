...
##############################################################################################
######################## TASK 3: BATCH NORM + RESIDUAL CONNECTIONS ###########################
##############################################################################################

# Add BatchNorm layer after each convolutional, before each ReLu


class ConvolutionalProcessingBlock_BatchNormed(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlock_BatchNormed, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict["batchnorm_0"] = nn.BatchNorm2d(out.shape[1])
        ...
        self.layer_dict["batchnorm_1"] = nn.BatchNorm2d(out.shape[1])
...
class ConvolutionalDimensionalityReductionBlock_BatchNormed(nn.Module):
    def __init__(
        self,
        input_shape,
        num_filters,
        ...
        super(ConvolutionalDimensionalityReductionBlock_BatchNormed, self).__init__()
...
# Do the 2nd convolutional, add x to out, then do the 2nd ReLu
