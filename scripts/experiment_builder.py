...
class ExperimentBuilder(nn.Module):
    def __init__(self, network_model, experiment_name, num_epochs, train_data, val_data,
                 test_data, weight_decay_coefficient, use_gpu, continue_from_epoch=-1, lr=0.001):
        """
    ...
    def plot_func_def(self,all_grads, layers):
        ...
        """
        non_BN_layers =[layer if layer[-11:-2]!='batchnorm'else''for layer in layers]
        plt.plot(all_grads, alpha=0.3, color="b")
        plt.hlines(0, 0, len(all_grads)+1, linewidth=1, color="k" )
        plt.xticks(range(0,len(all_grads), 1), non_BN_layers, rotation="vertical")
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for name,param in named_parameters:
            ...
            name=name_list[0]
            if len(name_list) > 2: 
                name=name_list[1]+'_'+name_list[3]
            if name not in name_mean_dict.keys():
                name_mean_dict[name] = []
            
            name_mean_dict[name].append(param.grad.abs().mean().cpu().numpy())        
        layers = name_mean_dict.keys()
        all_grads = [name_mean_dict[layer] for layer in layers]
        ########################################
